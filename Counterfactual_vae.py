...
class DecoderY(nn.Module):
    def __init__(self, U_y_dim, num_classes, hid_dim=3):
        super(DecoderY, self).__init__()
        self.fc1 = nn.Linear(U_y_dim + 3, hid_dim)
    ...
    def forward(self, U_y, A, S, C):
        ...
        logits = self.fc2(F.tanh(self.bn1(self.fc1(torch.cat([U_y, A, S, C], dim=1)))))
...
class EncoderY(nn.Module):
    def __init__(self, U_y_dim, num_classes, hid_dim=3):
        ...
        self.fc1 = nn.Linear(4, hid_dim)
        self.bn1 = nn.BatchNorm1d(hid_dim)
        self.bnv = nn.BatchNorm1d(hid_dim)
        self.m1 = nn.Linear(hid_dim, U_y_dim)
    ...
    def forward(self, YCAS):
        ...
        x = F.tanh(self.bn1(self.fc1(YCAS)))
        mean_y = self.m1(x)
        std_y = torch.exp(self.v2(F.tanh(self.bnv(self.v1(x)))))
        return mean_y, std_y
